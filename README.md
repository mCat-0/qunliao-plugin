...
| 12 | 📺 B站直播推送  | `bilitvPush.js`           | `#B站推送` / `#B站全部推送` / `#B站订阅列表`                                   | 订阅 B 站 UP 的直播提醒，可通过锅巴面板配置订阅列表、扫描时间、推送群、背景渐变滤镜等，仅管理员触发 |
| 13 | 🔄 插件更新      | `update.js`              | `#更新群聊插件` / `#修复群聊插件`                                          | 从 Git 仓库自动获取最新版本（支持 GitHub / Gitee / GitLab 自动回退），仅管理员可用 |
...
- **新增** B站直播推送模块 `bilitvPush.js`：可通过 `#B站推送` / `#B站全部推送` / `#B站订阅列表` 触发，支持订阅多个 B 站 UP 主并在开播时推送图片至白名单群
- **锅巴面板**新增：订阅 UID 列表、扫描时间、推送群白名单、夜间扫描开关与时间段、背景渐变滤镜开关、自定义渐变、底部蒙版强度等参数，配置保存后立即生效
- `package.json` 版本号提升至 `1.2.9`
